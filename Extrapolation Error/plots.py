...
def main(beta=1, B=0.06, phi=1, p_in = 0.5, cluster_selection = "bernoulli", estimators = ['PI-$n$($p$)', 'HT', 'DM-C', 'DM-C($0.75$)', 'PI-$n$($B$)', 'LS-Prop', 'LS-Num'] ): 
    n = 1000
    nc = 50
    p_out = (0.5-p_in)/49
...
if __name__ == "__main__":
    ...
    estimators = ['PI-$n$($p$)', 'HT', 'DM-C', 'DM-C($0.75$)', 'PI-$n$($B$)', 'LS-Prop', 'LS-Num']
    
    for b in range(len(beta)):
        print('Plotting degree: {} ({} design)'.format(beta[b], design))
        for phi in phis:
            print()
            main(beta[b], B, phi, p_in, design, estimators)