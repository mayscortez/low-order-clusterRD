...
def main():
    ...
    results = []
    sizes = np.array([2500, 3600, 4900, 6400, 8100, 10000, 12100])
...
def run_experiment(G,T,n,p,r,graphStr,diag=1,loadGraphs=False):
    ...
    for g in range(G):
        ...
        estimators.append(lambda y,z: (1/np.sum(z))*np.sum(y - fy(np.zeros(n))))
        estimators.append(lambda y,z: es.diff_in_means_naive(y,z))
        estimators.append(lambda y,z: es.diff_in_means_fraction(n,y,A,z,0.75))
