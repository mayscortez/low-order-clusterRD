...
def main(beta, graphNum, T, U, p=0.06, q=1, p_in=0.5, cluster_selection_RD = "bernoulli"):
    '''
    ...
    q (float): treatment probability if cluster is chosen (treatment prob conditioned on being in U)
    ...
    fixed = '_n' + str(n) + '_nc' + str(nc) + '_' + 'in' + str(np.round(p_in,3)).replace('.','') + '_out' + str(np.round(p_out,3)).replace('.','') + '_q' + str(np.round(q,3)).replace('.','') + '_p' + str(p).replace('.','') # naming convention
...
def run_experiment(beta, n, nc, p, q, r, diag, Pii, Pij, phi, design, EK, graphNum, T, U, graphStr, p_prime=0, realized = True):
    ...
    q = the treatment probability within clusters i.e. the target treatment probability - note this may differ from the realized t.p.
    ...
    dict_base = {'n': n, 'nc': nc, 'Pii': Pii, 'Pij': np.round(Pij,3), 'Phi': phi, 'p': p, 'q': q, 'EK': EK, 'ratio': r}
    ...
    alg_names_clusterRD = ['PI($q$)', 'PI-$\mathcal{U}$($p$)', 'HT', 'DM-C', 'DM-C($0.75$)']
