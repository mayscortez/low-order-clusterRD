...
def plot(data, col_var, row_var, outfile):
    ...
    if ncol == 1:
        ...
        ax.fill_between(df['q'], df['bias']**2, df['bias']**2 + df['var'], color=colors[1], hatch='\\\\', alpha=0.25, label="Variance")
        ...
        for j in range(ncol):
            if nrow == 1:
                ...
                ax[j].fill_between(cell_df['q'], cell_df['bias']**2, cell_df['bias']**2 + cell_df['var'], color=colors[1], hatch='\\\\', alpha=0.25, label="Variance")
                ...
                for i in range(nrow):
                    ...
                    ax[i,j].fill_between(cell_df['q'], cell_df['bias']**2, cell_df['bias']**2 + cell_df['var'], color=colors[1], hatch='\\\\', alpha=0.25, label="Extrapolation Variance")
