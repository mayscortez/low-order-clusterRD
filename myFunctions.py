...
def staggered_rollout_bern(N, P, lst):
  '''
  Returns Treatment Samples from Bernoulli Staggered Rollout

  N (int): size of population (N=n*n)
  P (numpy array): treatment probabilities for each time step
  lst (list): list of the nodes who were selected to be in the staggered rollout experiment
  '''

  ### Initialize ###
  Z = np.zeros(shape=(len(P),N))   # for each treatment sample z_t
